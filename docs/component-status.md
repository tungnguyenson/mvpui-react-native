...
| PinInput | ✅ | `packages/ui/src/components/pin-input.tsx`. `react-native-confirmation-code-field` v9 (`CodeField` + `useBlurOnFulfill` + `useClearByFocusCell`). Controlled: `value: string` + `onChangeText`. `length` (default 6). `onComplete(code)` fires when all cells filled. `isInvalid` turns all borders error-red. `hint: ReactNode` below cells. `secureTextEntry` masks with "•" at renderCell layer + underlying TextInput. Cells: 48×56px, `rounded-xl`, border-color flips focus/invalid/default via JS token maps (no NativeWind on SVG paths). `Cursor` from library animates the active empty cell. Imperative ref via `PinInputRef`: `focus()` / `blur()` / `clear()`. Demo: `apps/showcase/src/app/components/pin-input.tsx`. |
...
| StatusBar | ✅ | `packages/ui/src/components/status-bar.tsx`. Thin `expo-status-bar` wrapper with `style="auto"|"light"|"dark"|"inverted"` + `hidden` + `animated`. Auto resolves scheme to `light`/`dark` content color matching SafeArea's internal `statusBar` prop. Do NOT mount alongside SafeArea's own StatusBar. Demo: `apps/showcase/src/app/components/status-bar.tsx`. |
...
| CircularProgress | ✅ | `packages/ui/src/components/circular-progress.tsx`. `react-native-svg` Circle + Reanimated `useAnimatedProps` on `strokeDashoffset`. Track ring (muted, full) + fill ring (animated arc). Starts at 12 o'clock via `rotation={-90}`. API mirrors ProgressBar: `value` 0–100, `color` primary/success/warning/error, `showValue`, `animationDurationMs`. RN-only additions: raw `size` px (default 48) + `thickness` stroke px (default 4) + `label` center text override. JS color maps via `useColorScheme()` (SVG ignores className). Demo: `apps/showcase/src/app/components/circular-progress.tsx`. |
...
| Banner | ✅ | `packages/ui/src/components/banner.tsx`. Full-width inline bar (`border-t border-b`, no border-radius). 5 variants: `neutral` (default, `bg-bg-secondary`) + `info`/`success`/`warning`/`error` (semantic bg/border tokens). Props: `title` (required), `description`, `icon: IconProp`, `onDismiss` (adds 40pt X + `FadeOut.duration(200)` on unmount). Distinct from Alert (boxed, `rounded-xl`) by visual weight + neutral variant + placement (between Header and ScrollView). JS icon tint maps via `useColorScheme()`. Demo: `apps/showcase/src/app/components/banner.tsx`. |
...
| FAB | ✅ | `packages/ui/src/components/fab.tsx`. Visual primitive — NO built-in `position:absolute` (consumer handles placement). Sizes `md` (56px) / `lg` (64px). Colors `primary` (brand-600 fill, brand-tinted shadow matching `FabTabBar.tsx` convention) / `secondary` (bg fill, border) / `surface` (bg-secondary fill). `label` prop switches to extended FAB (pill, icon+text). `isLoading` shows Spinner. JS bg + icon tint maps via `useColorScheme()`. Recipe for tab-bar cutout: copy `apps/showcase/src/components/FabTabBar.tsx` (stays app-layer; expo-router dep). Demo: `apps/showcase/src/app/components/fab.tsx`. |
...
### Final design-system batch (2026-05-23) — landed

Closes all remaining ❌ items that belong in `packages/ui`. Deferred: Breadcrumb / Pagination / SideNav (low mobile utility), Table (redesign needed), Card variants / Onboarding (app-layer).

Decisions locked (Q1–Q4 confirmed before build):
- **Q1 — Banner positioning:** inline static (like Alert). Consumer controls visibility via React state. No singleton store. Placement: between Header and ScrollView content.
- **Q2 — CircularProgress API:** raw `size` px + ProgressBar-mirrored props (`value`, `color`, `showValue`, `animationDurationMs`) + RN-only `thickness` stroke prop. `react-native-svg` + Reanimated `useAnimatedProps` on `strokeDashoffset`.
- **Q3 — FAB scope:** visual primitive only — no built-in `position:absolute`. Consumer handles placement. `FabTabBar` stays app-layer (`apps/showcase/src/components/FabTabBar.tsx`) as a copy-paste recipe. Brand-tinted shadow on `primary` color matches established `FabTabBar.tsx` convention.
- **Q4 — PinInput contract:** controlled `value: string` + flexible `length` (default 6) + `onComplete(code)`. `react-native-confirmation-code-field` v9. `PinInputRef` exposes `focus()` / `blur()` / `clear()`.

Build order shipped: StatusBar → Banner → CircularProgress → FAB → PinInput. All pass `tsc --noEmit` across `packages/ui` / `apps/showcase`.

Infrastructure changes:
- `react-native-confirmation-code-field@^9.0.0` added to `packages/ui` deps + `apps/showcase` deps.
- No new portal hosts or providers needed.
