...
# Runs the Maestro batch-showcase flow twice (light + dark) and renames
# captures so both modes survive in .maestro-out/.

REPO_ROOT=$(cd "$(dirname "$0")/../../.." && pwd)
FLOW="$REPO_ROOT/apps/showcase/.maestro/batch-showcase.yaml"
...
# Wipe prior batch captures so the diff between runs is clean.
