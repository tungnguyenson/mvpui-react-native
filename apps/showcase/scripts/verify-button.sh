...
# Runs the Maestro button-showcase flow twice — once in light appearance,
...
# reliable than driving an in-app toggle (which the prior flow accidentally
# tripped Expo Go's developer menu when Maestro routed a long-press to the
# small icon-only toggle button).

REPO_ROOT=$(cd "$(dirname "$0")/../../.." && pwd)
FLOW="$REPO_ROOT/apps/showcase/.maestro/button-showcase.yaml"
...
run_mode() {
  ...
  # Give Expo Go time to relaunch and Metro to hand off the bundle. Light
  # cold start is ~5s, dark a touch slower.
  sleep 8
